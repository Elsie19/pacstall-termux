...
if [[ -f /data/data/com.termux/files/usr/tmp/pacstall-up-list ]]; then
	rm /data/data/com.termux/files/usr/tmp/pacstall-up-list
fi

if [[ -f /data/data/com.termux/files/usr/tmp/pacstall-up-print ]]; then
	rm /data/data/com.termux/files/usr/tmp/pacstall-up-print
fi

if [[ -f /data/data/com.termux/files/usr/tmp/pacstall-up-urls ]]; then
	rm /data/data/com.termux/files/usr/tmp/pacstall-up-urls
fi

touch /data/data/com.termux/files/usr/tmp/pacstall-up-list
touch /data/data/com.termux/files/usr/tmp/pacstall-up-print
touch /data/data/com.termux/files/usr/tmp/pacstall-up-urls
...
for i in "${list[@]}"; do
	...
	if [[ -n "$remotever" ]]; then
		if [[ $i == *"-git" ]] || ver_compare "$localver" "$remotever"; then
			echo "$i" | tee -a /data/data/com.termux/files/usr/tmp/pacstall-up-list >/dev/null
			echo "\t${GREEN}${i}${CYAN} @ $(parseRepo "${remoteurl}") ${NC}" | tee -a /data/data/com.termux/files/usr/tmp/pacstall-up-print >/dev/null
			echo "$remoteurl" | tee -a /data/data/com.termux/files/usr/tmp/pacstall-up-urls >/dev/null
		fi
	fi
done

if [[ $(wc -l /data/data/com.termux/files/usr/tmp/pacstall-up-list | awk '{ print $1 }') -eq 0 ]] ; then
	fancy_message info "Nothing to upgrade"
else
	fancy_message info "Packages can be upgraded"
	echo -e "Upgradable: $(wc -l /data/data/com.termux/files/usr/tmp/pacstall-up-print | awk '{ print $1 }')
${BOLD}$(cat /data/data/com.termux/files/usr/tmp/pacstall-up-print)${NORMAL}\n"

	upgrade=()
	while IFS= read -r line; do
		upgrade+=("$line")
	done < /data/data/com.termux/files/usr/tmp/pacstall-up-list

	while IFS= read -r line; do
		remotes+=("$line")
	done < /data/data/com.termux/files/usr/tmp/pacstall-up-urls

	export local='no'
	mkdir -p "$SRCDIR"
	chown -R "$PACSTALL_USER":"$PACSTALL_USER" "$SRCDIR"
...
if [[ -f "/data/data/com.termux/files/usr/tmp/pacstall-up-list" ]]; then
	rm -f /data/data/com.termux/files/usr/tmp/pacstall-up-list
fi

if [[ -f "/data/data/com.termux/files/usr/tmp/pacstall-up-print" ]]; then
	rm -f /data/data/com.termux/files/usr/tmp/pacstall-up-print
fi

if [[ -f "/data/data/com.termux/files/usr/tmp/pacstall-up-urls" ]]; then
	rm -f /data/data/com.termux/files/usr/tmp/pacstall-up-urls
fi
