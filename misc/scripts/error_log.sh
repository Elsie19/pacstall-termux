...
if [[ ! -d "/data/data/com.termux/files/usr/var/log/pacstall/error_log" ]]; then
...
function error_log() {
	...
	if [[ ! -f "$LOGFILE" ]]; then
		touch "$LOGFILE"
		find /data/data/com.termux/files/usr/var/log/pacstall/error_log/* -type f -ctime +14 -exec rm -rf {} \;
	fi
