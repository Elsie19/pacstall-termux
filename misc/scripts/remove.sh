...
if ! source /data/data/com.termux/files/usr/var/cache/pacstall/"${PACKAGE}"/"${_version}"/"${PACKAGE}".pacscript > /dev/null 2>&1; then
	fancy_message error "$PACKAGE is not installed or not properly symlinked"
...
case "$url" in
	*.deb)
		if ! apt remove -y "$gives" 2>/dev/null; then
	...
	*)
		...
		if ! apt purge -y "$name" 2> /dev/null; then
			fancy_message error "Failed to remove dummy package"
			error_log 1 "remove $PACKAGE"
			exit 1
		fi
		rm -f /data/data/com.termux/files/usr/etc/apt/preferences.d/"${name}-pin"
